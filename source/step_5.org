...
:HTML_CONTAINER_CLASS: learn
:END:
- *Silver:* Modify the code to draw the first square with size 20 sides, the second with size 40, third with 60 and so on. 
- *Gold:* Change the code so that the can enter five colours that are used to draw a pattern of your choice.
